...
Reuses `RETRIEVAL_BEARER_TOKENS` and `RETRIEVAL_BASE_URL` -- the same root
...
class Settings(BaseSettings):
    ...
    def bearer_token(self) -> str:
        # The service accepts any one of a comma-separated set (retrieval
        # spec.md NFR4); a caller only ever needs to present one.
