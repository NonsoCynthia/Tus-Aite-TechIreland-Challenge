...
capacity agent's *only* interface to Postgres/the graph. It never issues
SPARQL or Postgres queries directly.
"""
...
class RetrievalClient:
    ...
    token every other caller of that service uses (spec.md NFR4: the token
    carries no per-caller identity)."""
    ...
    def get_referral_context(self, hospital_hipe: str, pathway_number: str) -> dict[str, Any]:
        """FR9: everything the capacity agent needs to judge one referral."""
    ...
    def get_cohort(self, hospital_hipe: str, as_of_date: date) -> dict[str, Any]:
        ...
        find which referrals need a capacity score, not to compute one."""
    ...
    def get_scores_for_run(self, run_id: str, hospital_hipe: str) -> dict[str, Any]:
        ...
        verify `POST /scores`'s round trip (test_run_integration.py); the
        capacity agent itself never needs to read its own scores back."""
    ...
    def post_score(
        ...
        *,
        run_id: str,
        agent_name: Literal["urgency", "capacity"],
        hospital_hipe: str,
        pathway_number: str,
        as_of_date: date,
        score: float,
        ...
        ADR-002. Raises `GraphProjectionFailedError` on the service's 207
        (row written, graph projection failed) and `RetrievalServiceError`
        on any other non-2xx (400 means nothing was written at all)."""
        payload = {
            "run_id": run_id,
